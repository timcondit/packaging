...
[components]
A component is a zip file consisting of the installable file and a data file.


The formal name of a component is the 40-digit SHA-1 hexdigest calculated from
the contents of the file inside the component.

=== OR ===

The formal name of a component is the 40-digit SHA-1 hexdigest calculated from
the contents of the installable file and the data file inside the component.


  The name of the component zip file is 

The name of the file is <formal-name>.zip.

<aside>
Q: Is that the right way to go?  Should it be the hexdigest of the file plus
   it's metadata instead?
A: Answer the question: is the metadata operative?  For proper functioning of
   the system, the answer has to be yes.  So it's probably wise to include it
   or use all the contents of the component as the hexdigest formal name.  But
   if we hash the installable file and the data file, it's value will change
   every time there's a change to the data file.  That's very bad.  We could
   use two hashes per component: a stable file hash and a variable component
   hash.  That may be the way to go ... we'll see.
</aside>

The data file is called <formal-name>.txt.  It contains:
* 

* a formal name (
* an informal name

 (or a reference to the file only?)
